...
class AD9363(object):
    ...
    def __init__(self, uri=PLUTO_ID):
        ...
        # Sampling
        self.phy_rx_sampling = self.phy.find_channel('voltage', is_output=True) # RX0 sampling frequency + Quadrature (bool) + RF DC Correction (bool) + BB DC Correction (bool)
        self.phy_tx_sampling = self.phy.find_channel('voltage', is_output=True) # RX1 Sampling frequency
    ...
    def _set_rx_gain(self, rx0_value=None, rx1_value=None):
        ...
        if rx0_value is not None:
            ...
            self.phy_rx0.attrs['hardwaregain'].value = '{:2.3f} dB'.format(value)

        if rx1_value is not None:
            self.phy_rx1.attrs['gain_control_mode'].value = 'manual'
            self.phy_rx1.attrs['hardwaregain'].value = '{:2.3f} dB'.format(value)
    ...
    def _set_rx_gain_mode(self, Rx0Value=None, Rx1Value=None):
        ...
        _mode = '' if len(Rx0Value) == 0 else mode[0].upper()
    ...
    def _set_rx_gain_mode(self, Tx0Value=None, Rx1Value=None):
        """set the gain mode to one of those available"""
        ...
        avail = avail.split()
        # allow setting with just the first letter
        _mode = '' if len(Tx0Value) == 0 else mode[0].upper()
        options = [av.capitalize()[0] for av in avail]
        if _mode in options:
            res = avail[options.index(_mode)]
        ...
        avail = avail.split()
        # allow setting with just the first letter
        _mode = '' if len(Rx1Value) == 0 else mode[0].upper()
        options = [av.capitalize()[0] for av in avail]
        if _mode in options:
            res = avail[options.index(_mode)]
    ...
    def _set_txBW(self, tx0_value=None, tx1_value=None):
        if tx0_value is not None:
            self.phy_tx0.attrs['rf_bandwidth'].value = _M2Str(value)
        if tx1_value is not None:
            self.phy_tx1.attrs['rf_bandwidth'].value = _M2Str(value)
    ...
    def _get_quadrature(self):
        ...
        return self.phy_rx_sampling.attrs['quadrature_tracking_en'].value
    def _set_quadrature(self, value):
        """set quadrature tracking"""
        self.phy_rx_sampling.attrs['quadrature_tracking_en'].value = value

    # RF DC Correction Offset (in_voltage_rf_dc_offset_tracking_en)
    # True/False
    def _get_rf_dc_offset(self):
        """return RF DC offset tracking"""
        return self.phy_rx_sampling.attrs['rf_dc_offset_tracking_en'].value
    def _set_rf_dc_offset(self, value):
        """set RF DC offset tracking"""
        self.phy_rx_sampling.attrs['rf_dc_offset_tracking_en'].value = value

    # BB DC Correction (in_voltage_bb_dc_offset_tracking_en)
    # True/False
    def _get_bb_dc_offset(self):
        """return BB DC offset tracking"""
        return self.phy_rx_sampling.attrs['bb_dc_offset_tracking_en'].value
    def _set_bb_dc_offset(self, value):
        """set BB DC offset tracking"""
        self.phy_rx_sampling.attrs['bb_dc_offset_tracking_en'].value = value
...
def main():
    ...
    # Corrections
    print("Quadrature", ad9363._get_quadrature())
    print("RF DC Offset", ad9363._get_rf_dc_offset())
    print("BB DC Offset", ad9363._get_bb_dc_offset())
