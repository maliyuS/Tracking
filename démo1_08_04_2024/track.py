...
from dsp_utils import scan_for_DOA, scan_for_DOAV2, CallCounter, SlidingWindowAverager
...
class CustomSDR(adi.ad9361):
    def __init__(self, uri):
        ...
        # Sampling
        self.sample_rate = 1e6  # must be <=30.72 MHz if both channels are enabled
...
def main():
    ...
    averager = SlidingWindowAverager()
    ...
    while (True):
        ...
        ''' DSP '''
        scan_DOA = scan_for_DOA(data['Rx_0'], data['Rx_1'], my_sdr.phase_cal, my_sdr.d, my_sdr.rx_lo)
        averager.add_sample(scan_DOA)

        if averager.is_window_full():
            '''Update UI'''
            average_values = averager.get_average()
            #fig.update_SCAN_window(scan_DOA)
            app.update_Gyroscope_window(average_values['steer_angle'])
